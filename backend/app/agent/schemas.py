...
Всё, что «течёт» через агента, описано здесь как pydantic-модели: на входе
`StylistRequest`, на выходе `StylistResult`. API-слой и сервис знают только эти
типы, а не внутренности агента.
"""
...
class StylistAction(str, enum.Enum):
    """Что агент решил сделать по итогам обработки запроса."""
