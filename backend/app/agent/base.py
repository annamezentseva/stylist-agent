...
Сначала договариваемся о контрактах (LLM, ретривер, каталог, vision, сам
агент), а конкретные реализации — заглушка или боевой режим — приходят потом и
взаимозаменяемы. Верхний код зависит только от этих абстрактных классов, а не
от httpx, SQLAlchemy или OpenAI.
"""
...
class Agent(ABC):
    """Сам агент-стилист. Вся работа — через один метод `handle`."""
