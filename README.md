...
- **Каталог — 13 вещей** из файла `data/catalog.json`, не живой API магазина.
  Поэтому на узкий бюджет подбор бывает грубоват.
...
Собрать архив только с исходниками — без ключей, окружений и библиотек:

```bash
cd /Users/mac/Documents/py_agents && git archive --format=zip -o ~/Desktop/stylist.zip HEAD:"Стилист"
```

`git archive` выгружает то, что под контролем версий, поэтому `.env` с ключами,
`.venv`, `node_modules` и файл базы в архив не попадают. Получателю хватит
инструкции из раздела «Быстрый старт» — проект запускается без ключей.
